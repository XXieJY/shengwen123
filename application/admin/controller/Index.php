...
class Index extends Admin
{
    ...
    public function checkUpdate()
    {
        ...
        $params['companyname']  = config('ieasynet.company_customer');
        $params['companykey'] = config('ieasynet.company_key');
        $params['product_version'] = config('ieasynet.product_version');
        ...
        if ($result['code'] == 1) {
            return json([
                'update' => '<a class="badge badge-primary" href="'.url('startupdate').'" target="_blank">最新版本：'.$result["version"].'</a>',
                'auth'   => $result['auth']
            ]);
        } else {
            return json([
    ...
    */
    public function startupdate(){
        ...
        if($result['upkey']==config('ieasynet.company_key'))
        {
            foreach($result['content'] as $key=>$value)
            {
                if($value['class']==1)
                {
                    $this->createDir(dirname($value['path'])); 
                    try
                    {
                        file_put_contents($value['path'],$value['content']);
                        echo $value['path']."写入成功！<br>";
                    }
                    catch (\Exception $e)
                    {
                        file_put_contents("updatelog.txt", $value['path']."\r\n",FILE_APPEND);
                        echo $value['path']."写入失败！<br>";
                    }
                }
                if($value['class']==2)
                {
                        $sql=explode(";\r\n",$value['content']);
                        foreach ($sql as $keysql => $sqldata) {
                            try
                            {
                                Db::execute($sqldata);
                                echo $sqldata."写入成功！<br>";
                            }
                             catch (\Exception $e)
                            {
                                file_put_contents("updatelog.txt", $value['path']."\r\n",FILE_APPEND);
                                echo $sqldata."写入失败！<br>";
                            }
                        }
                ...
                if($value['class']==3)
                {
                   $this->createDir(dirname($value['path'])); 
                    ...
                    try
                    {
                        $this->download($value['content'],$value['path']);
                        echo $value['path']."写入成功！<br>";
    ...
    function download($url=null, $path = null)
    {
      $ch = curl_init();
