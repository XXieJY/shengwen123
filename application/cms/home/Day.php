...
{  
    ...
    {  
        ...
        $output->writeln("正在更新提现记录请等待:");
        $this->cashlog();
        $output->writeln("提现记录更新成功");  
    }  
    public function cashlog(){
    	ignore_user_abort(true); // 后台运行
		set_time_limit(0); // 取消脚本运行时间的超时上限
		$user=DB::table('ien_admin_user')->where('role','<>','3')->where('money > 0')->select();
		foreach($user as $key=>$value)
		{
			$data['money']=$value['money'];
			$data['uid']=$value['id'];
			$data['type']=0;
			$data['bili']=$value['fcbl'];
			$data['create_time']=time();
			DB::table('ien_settlement_log')->insert($data);
			DB::table('ien_admin_user')->where('id',$value['id'])->update(['money'=>'0']);
		}
    ...
    public function allpay(){
		...
		foreach($user as $key=>$value)
		{
					...
					/*start*/
					$datey=date('Y',strtotime("-1 day"));
					$datem=date('m',strtotime("-1 day"));
					$dated=date('d',strtotime("-1 day"));
			...
			$monthlog=DB::table("ien_day_money")->whereTime('addtime', 'between',[$beginToday,$endToday])->where('uid',$value['id'])->find();
				...
				//log
				file_put_contents("dayerrorlog.txt",date($beginToday,'Ymd')."代理商ID:".$value['id']."订单统计更新失败\r\n",FILE_APPEND);
				sleep(1);
				}
    ...
    public function alluser($id=null){
		...
		$data['payuser']+=$user_xiaofei;
			...
			else
			{
				...
				//log
				file_put_contents("dayerrorlog.txt",date($beginToday,'Ymd')."代理商ID:".$value['id']."用户统计更新失败\r\n",FILE_APPEND);
				sleep(1);
				}
	...
	public function alllog(){
			...
			else
			{
				...
				//log
				file_put_contents("dayerrorlog.txt",date($beginToday,'Ymd')."平台统计更新失败\r\n",FILE_APPEND);
				sleep(1);
				}	
	...
	public function updatebookid(){
		ignore_user_abort(true); // 后台运行
		set_time_limit(0); // 取消脚本运行时间的超时上限
		$userlog=DB::query("select * from ien_agent where ljlx <> 3 and bid=0");
			...
			set_time_limit(0); // 取消脚本运行时间的超时上限
					...
					foreach ($book as $k => $valueb) {
						try{
								...
								//log
								file_put_contents("dayerrorlog.txt",date($beginToday,'Ymd')."小说ID".$valueb['bid']."小说统计更新失败\r\n",FILE_APPEND);
								sleep(1);
								}	
