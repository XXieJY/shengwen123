...

{
    ...
    $user=DB::table('ien_admin_user')->where('username',$openid)->find();    
  ...
  if($result_pay == "SUCCESS"){
    ...
    //payAfter($llpayNotify->notifyResp);
     ...
     // 使用通知里的 "微信支付订单号" 或者 "商户订单号" 去自己的数据库找到订单  
      ...
      //$no_order 订单号
      //$order['uid'] 用户openid
      //$this->checktempay($no_order,$order['uid']);
        ...
        $response = $app->payment->handleNotify(function($notify, $successful){  
           ...
           $day=$infodata['day'];
            ...
            if ($successful) {  
              ...
              ///通知
      //$notify->out_trade_no 订单号
      //$order['uid'] 用户openid
      //file_put_contents("ddd.txt", $noid."/".$order['uid']);
      //$this->checktempay($noid,$order['uid']);
              //$this->checktempay("BOOK151686547598524","oZN9Q1NSkKFBCwnvCEKrN0jHHQ9Y");
...
$userinfo=DB::table('ien_admin_user')->where('openid',$_SESSION['wechat_user']['original']['openid'])->find();
      ...
      /************************************************************/
      if(empty($_SESSION['target_url']))
        {$tzurl="/index.php/cms/pay/index/";}
      else
        {$tzurl=$_SESSION['target_url'];}

      $this->reqHandler->setReqParams($orderinfo,array('method'));
        ...
        //通知地址，必填项
        $this->reqHandler->setParameter('notify_url','http://'.module_config('agent.agent_payurl').'/index.php/cms/pay/wftpaysuccess/');
        //通知回调地址，目前默认是空格，商户在测试支付和上线时必须改为自己的，且保证外网能访问到
        $this->reqHandler->setParameter('callback_url',$tzurl);
        $this->reqHandler->setParameter('nonce_str',mt_rand(time(),time()+rand()));//随机字符串，必填项，不长于 32 位
        $this->reqHandler->createSign();//创建签名
        $dataconfig = \Utils::toXml($this->reqHandler->getAllParameters());        
          
        $this->pay->setReqContent($this->reqHandler->getGateURL(),$dataconfig);
        if($this->pay->call()){
            ...
            if($this->resHandler->isTenpaySign()){
                ...
                if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){
                    ...
                    $url="https://pay.swiftpass.cn/pay/jspay?token_id=".$this->resHandler->getParameter('token_id')."&showwxtitle=1";
                    Header("Location: $url"); 
                    exit();
                }else{
    ...
    //noid 充值订单号
    //openid 用户openid
    public function checktempay($noid=null,$openid=null){
      if ($noid === 0) $this->error('参数错误');
      if ($openid === 0) $this->error('参数错误');
      //查用户
      $user=DB::table('ien_admin_user')->where('openid',$openid)->find();
      //查上线是否开启通知
      $userdl=DB::table('ien_wechat_uconfig')->where('uid',$user['sxid'])->where('isopen',"on")->where('ispay',"on")->find();
      if(empty($userdl) || empty($userdl['tempay']) || empty($userdl['tempid']))
      {
        //没开或者内容没有。结束
        return true;
      }
      ...
      $config = [
                    ...
                    $config = array_merge($config, $config2);
                        ...
                        $no=DB::table('ien_pay_log')->where('payid',$noid)->find();

                        $tempinfoatt=preg_replace('/{nickname}/',$user['nickname'],$userdl['tempay']);
                        $tempinfoatt=preg_replace('/{score}/',$user['score'],$tempinfoatt);
                        $tempinfoatt=preg_replace('/{money}/',$no['money'],$tempinfoatt);
                        $tempinfoatt=preg_replace('/{time}/',date("Y-m-d H:i:s",$no['paytime']),$tempinfoatt);
    ...
    public function scanpay($id=null,$error=null){
     ...
     session_start();
      ...
      //添加代理ID和渠道ID
      $tgid=DB::table('ien_admin_user')->where('username',$_SESSION['wechat_user']['original']['openid'])->value('tgid');
      if(!empty($tgid))
      {
        $ddid=DB::table('ien_agent')->where('id',$tgid)->value('uid');
        if(!empty($ddid)){
      ...
      $userinfo=DB::table('ien_admin_user')->where('username',$_SESSION['wechat_user']['original']['openid'])->find();
      ...
      $data = [
       ...
       //判断超期用户订单
      $isout=DB::table('ien_admin_user')->where('username',$_SESSION['wechat_user']['original']['openid'])->value('isout');
      if($isout==1)
      {
        $data['isout']=1;
      }
      ...
      if($bili>0)
      {
        $sxid=DB::table('ien_admin_user')->where('username',$_SESSION['wechat_user']['original']['openid'])->value('sxid');
        if(!in_array($sxid,$nokou)  || empty($nokou['0']))
        {
          $kl=mt_rand(1,$bili);
          if($kl<=2)
    ...
    $product = [  
        ...
        'notify_url'         => 'http://'.module_config('agent.agent_payurl').'/index.php/cms/pay/scanpaysuccess', 
    ...
    $this->assign('json', $json);
    $this->assign('title','书币充值 - '.$data["money"].' 元');

    $this->assign('ordsn', $orderid); 
    if(empty($_SESSION['target_url']))
      {$url="/index.php/cms/pay/index/";}
    else
      {$url=$_SESSION['target_url'];}
    ...
    public function getstatus($order_no=null){
      session_start();
      if(empty($_SESSION['target_url']))
      {$url="/index.php/cms/user/index/";}
      else
      {$url=$_SESSION['target_url'];}
      $is=DB::table('ien_pay_log')->where('payid',$order_no)->where('status',1)->find();
      if(!empty($is))
      {
        $data['status']=1;
        $data['url']=$url;
    ...
    public function scanpaysuccess(){  
        ...
        $response = $app->payment->handleNotify(function($notify, $successful){  
           ...
           $day=$infodata['day'];
            ...
            if ($successful) {  
              ...
              Db::table('ien_admin_user')->where( 'username' , $order['uid'] )->setInc('score', $score);
              //更新openid
              /*if(!empty($notify->openid))
              {
                Db::table('ien_admin_user')->where( 'username' , $order['uid'] )->update(['openid' => $notify->openid]);
              }*/

              //增加VIP天数
              if($typeday==2){
                $uinfo=Db::table('ien_admin_user')->where( 'username' , $order['uid'] )->find();
                if($uinfo['isvip']=0 || $uinfo['vipetime']<time())
                {
                  $datatimer=time()+$day*86400;
                Db::table('ien_admin_user')

                ->where( 'username' , $order['uid'])
      ...
      //$this->checktempay($noid,$order['uid']);
        ...
        {
        //充值成功给代理商增加余额
        $dl=DB::table('ien_admin_user')->where( 'username' , $order['uid'] )->find();
        if($dl['tgid'])
        {
          $tg=DB::table('ien_agent')->where('id',$dl['tgid'])->find();
          if($tg['uid'])
